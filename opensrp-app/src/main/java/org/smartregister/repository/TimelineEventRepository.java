...
public class TimelineEventRepository extends DrishtiRepository {
    private static final String TIMELINEEVENT_SQL = "CREATE TABLE timelineEvent(caseID VARCHAR, "
            + "type VARCHAR, referenceDate VARCHAR, title VARCHAR, detail1 VARCHAR, detail2 VARCHAR)";
    private static final String TIMELINEVENT_CASEID_INDEX_SQL = "CREATE INDEX "
            + "timelineEvent_caseID_index ON timelineEvent(caseID);";
    ...
    private static final String[] TIMELINEEVENT_TABLE_COLUMNS = {CASEID_COLUMN,
            TYPE_COLUMN, REF_DATE_COLUMN, TITLE_COLUMN, DETAIL1_COLUMN, DETAIL2_COLUMN};
    ...
    public List<TimelineEvent> allFor(String caseId) {
        ...
        Cursor cursor = database.query(TIMELINEEVENT_TABLE_NAME,
    ...
    private List<TimelineEvent> readAllTimelineEvents(Cursor cursor) {
        ...
        while (!cursor.isAfterLast()) {
            timelineEvents.add(new TimelineEvent(cursor.getString(0),
                    cursor.getString(1),
