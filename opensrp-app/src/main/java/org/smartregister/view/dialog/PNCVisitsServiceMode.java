...
public class PNCVisitsServiceMode extends ServiceModeOption {
        ...
        return new ClientsHeaderProvider() {
            ...
            public int[] headerTextResourceIds() {
                return new int[]{R.string.header_name, R.string.header_thayi_number, R.string
                        .header_days_pp, R.string.header_delivery_complications, R.string
                        .header_first_seven_days, R.string.header_pnc_visits};
            }
        };
    }
    ...
    private void setUpPNCVisitLayout(PNCSmartRegisterClient client,
        ...
        if (!pncVisitAlert.equals(emptyAlert)) {
            ...
            viewHolder.layoutPNCVisitAlert()
                    .setOnClickListener(launchForm(client, pncVisitAlert, PNC_VISIT));
            setAlertLayout(viewHolder.layoutPNCVisitAlert(), viewHolder.txtPNCVisitDueType(),
                    viewHolder.txtPNCVisitAlertDueOn(), pncVisitAlert);
        } else {
            viewHolder.btnPncVisitView().setVisibility(View.VISIBLE);
            viewHolder.layoutPNCVisitAlert().setVisibility(VISIBLE);
            viewHolder.layoutPNCVisitAlert()
    ...
    private View.OnClickListener launchForm(PNCSmartRegisterClient client, AlertDTO alert, String
            formName) {
        return provider().newFormLauncher(formName, client.entityId(),
                "{\"entityId\":\"" + client.entityId() + "\"}");
    }

    private void setUpPNCRecentVisits(PNCSmartRegisterClient client,
                                      NativePNCSmartRegisterViewHolder viewHolder) {
        viewHolder.recentPNCVisits().removeAllViews();
        for (ServiceProvidedDTO serviceProvided : client.recentlyProvidedServices()) {
            ViewGroup recentVisitsGroup = (ViewGroup) inflater
                    .inflate(R.layout.smart_register_pnc_recent_visits_layout, null);
            ((TextView) recentVisitsGroup.findViewById(R.id.txt_recent_visit_day))
                    .setText(String.valueOf(serviceProvided.day()));
            ((TextView) recentVisitsGroup.findViewById(R.id.txt_recent_visit_date))
                    .setText(serviceProvided.dateForDisplay());
            viewHolder.recentPNCVisits().addView(recentVisitsGroup);
        }
    }

    private void setupDaysPPView(PNCSmartRegisterClient client, NativePNCSmartRegisterViewHolder
            viewHolder) {
        viewHolder.txtNumberOfVisits().setText(
                String.valueOf(DateUtil.dayDifference(client.deliveryDate(), DateUtil.today())));
    ...
    private void setUpPNCVisitsGraph(PNCSmartRegisterClient client,
                                     NativePNCSmartRegisterViewHolder viewHolder) {
        String jsonString = new Gson()
                .toJson(client.firstSevenDaysVisits(), PNCFirstSevenDaysVisits.class);
        viewHolder.wbvPncVisitsGraph()
                .loadUrl("javascript:drawSevenDayGraphic('" + jsonString + "')");
    }
